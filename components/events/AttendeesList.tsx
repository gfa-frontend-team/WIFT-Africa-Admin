        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-border">
            {data.attendees.map((attendee, index) => {
              // Fallback to _id or index if id is missing
              const uniqueKey = attendee.user?.id || (attendee.user as any)?._id || `attendee-${index}`

              return (
                <tr key={uniqueKey} className="hover:bg-muted/30">
