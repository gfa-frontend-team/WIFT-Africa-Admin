...
export function MemberProfileModal({ member, isOpen, onClose, onStatusChange }: MemberProfileModalProps) {
  ...
  const handleStatusUpdate = async (newStatus: 'APPROVED' | 'SUSPENDED') => {
    ...
    try {
      ...
      setReason('')
      if (newStatus === 'SUSPENDED') onClose() 
    } catch (error: any) {
  ...
  // Derived state for profile from potential populated fields
  // Note: The User type in frontend might not always have profile populated depending on endpoint
  // But let's assume standard User object or check if we need to fetch profile separately.
  // The members list usage usually sends basic user info. 
  // If we need full profile (like bio/roles), we might need to fetch it.
  // For now, let's display what we have in User object + Profile if attached.
  // Checking User type again... 
  // User has `profile?: Profile`. If it's not populated, we show empty states.
              ...
              <div className="flex gap-2">
                ...
                {member.membershipStatus === 'SUSPENDED' ? (
                  ...
                  <Button 
                    ...
                    disabled={member.accountType === 'SUPER_ADMIN'} // Prevent suspending super admins easily?
                  >
                    Suspend Member
                  </Button>
                )}
      ...
      <Dialog open={isSuspendDialogOpen} onOpenChange={setIsSuspendDialogOpen}>
        <DialogContent>
          ...
          <div className="my-4">
             ...
             <textarea
               className="w-full px-3 py-2 border border-input rounded-lg bg-background min-h-[100px]"
