...
import { FundingOpportunity, ApplicationType, FundingStatus } from "@/types";
...
interface FundingCardProps {
  ...
  onDelete: (opportunity: FundingOpportunity) => void;
}

// ✅ MEMOIZED: FundingCard component
const FundingCard = ({ opportunity, onEdit, onDelete }: FundingCardProps) => (
      ...
      <div className="flex flex-col sm:flex-row p-6 gap-6">
        ...
        <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <div className="flex gap-2">
                  ...
                  <Badge
                    variant={
                      opportunity.status === FundingStatus.OPEN
                        ? "default"
                        : "secondary"
                    }
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                ...
                opportunity.targetRoles.length > 0 ? (
                  <>
                    ...
                    {opportunity.customRoles &&
                      opportunity.customRoles.length > 0 && (
                        <>
                          {opportunity.customRoles.map((role, idx) => (
                            <Badge
                              key={`custom-${idx}`}
                              variant="outline"
                              className="text-[10px]"
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground pt-2 border-t border-border">
            ...
            {opportunity.applicationType === ApplicationType.INTERNAL && (
              <div className="flex items-center gap-2">
            ...
            {opportunity.applicationType === ApplicationType.REDIRECT &&
        ...
        <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 border-t sm:border-t-0 sm:border-l border-border pt-4 sm:pt-0 sm:pl-6 mt-2 sm:mt-0">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem>
                <Link
                  href={`/dashboard/funding/${opportunity._id}`}
...
export default function FundingPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["funding", debouncedSearch, isChapterAdmin, userChapterId],
    queryFn: () =>
      fundingApi.getAllOpportunities({
        page: 1,
        limit: 100,
  ...
  // ✅ MEMOIZED: Filter opportunities by tab
  const filteredOpportunities = useMemo(() => {
    if (!data?.data) return [];

    switch (activeTab) {
      case "open":
        return data.data.filter((opp) => opp.status === FundingStatus.OPEN);
      case "closed":
        return data.data.filter((opp) => opp.status !== FundingStatus.OPEN);
      default:
        return data.data;
  ...
  const tabCounts = useMemo(() => {
    if (!data?.data) return { all: 0, open: 0, closed: 0 };

    return {
      all: data.data.length,
      open: data.data.filter((opp) => opp.status === FundingStatus.OPEN).length,
      closed: data.data.filter((opp) => opp.status !== FundingStatus.OPEN)
        .length,
    };
  }, [data?.data]);
  ...
  // ✅ MEMOIZED: Callbacks
  const handleEdit = useCallback((opportunity: FundingOpportunity) => {
  ...
  }, [opportunityToDelete, deleteMutation]);

  const handleDeleteClick = useCallback((opportunity: FundingOpportunity) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between gap-4 items-start sm:items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage external and internal funding opportunities
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="open" className="mt-6">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground bg-accent/20 rounded-lg border border-dashed">
              <p>No open funding opportunities</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {filteredOpportunities.map((opportunity, index) => (
        ...
        <TabsContent value="closed" className="mt-6">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground bg-accent/20 rounded-lg border border-dashed">
              <p>No closed funding opportunities</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {filteredOpportunities.map((opportunity, index) => (
      ...
      <AlertDialog
        ...
        onOpenChange={(open: boolean) => !open && setOpportunityToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              funding opportunity &quot;{opportunityToDelete?.name}&quot;.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setOpportunityToDelete(null)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
