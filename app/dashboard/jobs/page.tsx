...
export default function JobsPage() {
  ...
  const isChapterJob = (posterId:string,jobChapterId:string | undefined) =>posterId === data?.adminId && data?.chapterId === jobChapterId;

  // Data Fetching
  const { data: jobsResponse, isLoading } = useJobs(filters);
  const jobs = jobsResponse?.data || [];

  // console.log(isSuperAdmin, "jobs");
            ...
            <Card key={job.id}>
              ...
              <CardFooter className="flex justify-between border-t pt-4">
                ...
                <div className="flex gap-2">
                  {/* {canManage && ( */}
                  <>
                    { (isSuperAdmin||isChapterJob(job.createdBy,job?.chapterId)) && (
                      <Button
                        size="sm"
                        variant="outline"
