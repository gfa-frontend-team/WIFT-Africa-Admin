...
export const eventSchema = z.object({
  ...
  location: z.object({
    ...
    if (data.type === LocationType.PHYSICAL || data.type === LocationType.HYBRID) {
      if (!data.address) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Address is required for physical/hybrid events', path: ['address'] })
    ...
    if (data.type === LocationType.VIRTUAL || data.type === LocationType.HYBRID) {
      if (!data.virtualUrl) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Virtual URL is required for virtual/hybrid events', path: ['virtualUrl'] })
    }
  }),
